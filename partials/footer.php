...
if ('post' === get_post_type()) {
	...
	printf(
		...
		_x('Author', 'Used before post author name.', 'TEXT_DOMAIN'),
...
if (in_array(get_post_type(), ['post', 'attachment'])) {
	printf(
		...
		get_the_date('c'),
		sprintf(
			__('Published on %1$s at %2$s', 'TEXT_DOMAIN'),
			get_the_date(),
			get_the_time()
		)
	);
...
if ('post' === get_post_type()) {
	$categories_list = get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'TEXT_DOMAIN'));
	if ($categories_list && count($categories_list) > 1) {
		printf(
			'<span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x('Categories', 'Used before category names.', 'TEXT_DOMAIN'),
			$categories_list
		);
	}

	$tags_list = get_the_tag_list('', _x(', ', 'Used between list items, there is a space after the comma.', 'TEXT_DOMAIN'));
	if ($tags_list && count($tags_list) > 1) {
		printf(
			'<span class="tags-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x('Tags', 'Used before tag names.', 'TEXT_DOMAIN'),
			$tags_list
		);
	}
}
